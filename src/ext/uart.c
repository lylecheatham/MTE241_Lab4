...
**						clock, parity, stop bits, FIFO,
*etc.
**
** parameters:			portNum(0 or 1) and UART baudrate
** Returned value:		true or false, return false only if the
**						interrupt handler can't be
...
uint32_t UARTInit(uint32_t PortNum, uint32_t baudrate) {
    ...
    if (PortNum == 0) {
        ...
        LPC_UART0->LCR = 0x83; /* 8 bits, no Parity, 1 Stop bit, The access to
                                  Divisor latches is enabled. */
        ...
        // LPC_UART1->IER = IER_RBR | IER_THRE | IER_RLS;	/* Enable UART1
        // interrupt
        // */
...
**
** Descriptions:		Recieve a block of data to the UART 0-1 port
...
uint32_t UARTRecieve(uint32_t portNum, uint8_t *BufferPtr, uint32_t Length) {
    ...
    *UARTCount = 0x0;
    ...
    for (i = 0; i < *UARTCount; ++i) rcvdBufferPtr[rcvd_len++] = UARTBuffer[i];
