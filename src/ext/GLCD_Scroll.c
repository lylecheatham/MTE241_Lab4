...
uint8_t chache[CACHE_LINE_CAP]
              [LCD_WIDTH + 1];  // The chach size is CACHE_LINE_CAP lines of
                                // LCD_WIDTH + 1 length (Each line is an string,
                                // and string ends up to '0').

// The starting index of the cache
uint32_t cache_start = 0;
// How many lines are filled so far			Therefore, the last line
// that can be wirtten is
size_t cache_size = 0;
...
void refresh_lcd(void) {
    ...
    for (i = 0; i <= window_size; ++i) {
        GLCD_DisplayString(i, 0, 1,
                           chache[(i + window_start) % CACHE_LINE_CAP]);
    }
}

/*
        This function prints and records the input character.
just_record:		If just_record is true, the character won't be printed
and just recorded  in the cache.
*/
void append_char(uint8_t _char) {
    ...
    if (_char == '\n') {
        ...
        GLCD_DisplayChar(last_line_to_append, last_col_cahche, FONT_SIZE,
                         _char);
